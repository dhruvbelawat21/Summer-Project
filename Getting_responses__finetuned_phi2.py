...
def dict_to_frame(text):
    data = []
    for question_id, question_details in text.items():
        data.append({
...
preamble = 'Answer the following question by selecting the most likely answer choice (A, B, C, or D): please generate only answer choice'
...
model = AutoModelForCausalLM.from_pretrained("dhruvbelawat21/SummerProject_Finetuned_phi2", torch_dtype="auto", trust_remote_code=True).to(device)
tokenizer = AutoTokenizer.from_pretrained("dhruvbelawat21/SummerProject_Finetuned_phi2", trust_remote_code=True)
    ...
    for line in lines:
        if line.startswith("Answer:"):
            try:
                ...
                return 'D'  # Default to 'D' if splitting fails
    return 'D'  # Default to 'D' if no line starts with "Answer:"
...
def get_ans():
    ...
    for i in tqdm(range(test_df.shape[0])):
        ...
        #print(f"Model output for question {i}: {text}")  # Debug information
